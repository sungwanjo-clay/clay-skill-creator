...
def extend(graph,account_id):
 if not isinstance(account_id,str) or not account_id.startswith('aa_') or any(c.isspace() for c in account_id):
  raise ValueError('Select an explicit Clay HTTP header account ID, not an Exa API key')
 ...
 def http(key,name,body,parent):
  return {'key':key,'spec':{'nodeType':'tool','name':name,'inputSchema':ins(body=('exa_prepare','$.'+body)),
   'tools':[{'toolType':'clay_action','actionKey':HTTP_ACTION,'actionPackageId':HTTP_PACKAGE,'appAccountId':account_id,
