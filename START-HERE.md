...
**You always get both ways to submit, and picking is yours.** Either you upload it through the Clay
Marketplace submission form, or the agent sends it from the session. **Naming both is required of the
flow; sending is never required of you** — those are different things, and only the first is
...
**One thing to read before you send, while it is still true (written 2026-08-22).** The review queue
a submission lands in is readable without credentials until authentication ships next week, and your
submission carries your name and work email. Nothing publishes without an approval, and there is no
self-service withdrawal yet — ask whoever gave you the link. `SUBMITTING.md` has the whole picture
under *Before you send anything*, and that section is dated to be deleted rather than left to age.
